...
def get_data():   
    ...
    csv_file = open('F:\\xdl\\github\\crawler-demo\\zkh\\data\\zkh_cate.csv','w')
    ...
    for i in range(0,1):
        ...
        print('----------------------------------')
        str = 'category:' + firstCatalog[0]['catalogName'] + ',' + firstCatalog[1]['catalogName']
        if len(firstCatalog)==3:
            str += ',' + firstCatalog[2]['catalogName']
        print(str)

        childrenCatalog = cate_json[i]['childrenCatalog']

        for j in range(0,1): # len(childrenCatalog)
